...
async def upload_omr_sheet(
    ...
    return await save_omr_sheet(file, label)

@router.post("/omr/align-check", response_model=OmrAlignCheckResponse)
async def check_omr_align_endpoint(file: UploadFile = File(...)) -> OmrAlignCheckResponse:
    ...
    # canh may khop giay chua truoc khi tu dong chup that.
...
async def get_omr_sheets() -> list[OmrSheetResponse]:
    return await list_omr_sheets()

@router.get("/omr/sheets/{sheet_id}/file")
async def get_omr_sheet_file_endpoint(sheet_id: str) -> Response:
    file_bytes, content_type = await get_omr_sheet_file(sheet_id)
    return Response(content=file_bytes, media_type=content_type)

@router.delete("/omr/sheets/{sheet_id}")
async def delete_omr_sheet_endpoint(sheet_id: str) -> Response:
    await delete_omr_sheet(sheet_id)
    return Response(status_code=204)

@router.get("/omr/sheets/{sheet_id}/aligned")
async def get_omr_sheet_aligned_endpoint(sheet_id: str) -> Response:
    image_bytes, was_aligned = await get_omr_sheet_aligned_bytes(sheet_id)
    return Response(
        content=image_bytes,
...
async def create_omr_template_endpoint(request: OmrTemplateCreateRequest) -> OmrTemplateResponse:
    return await create_omr_template(request)

@router.get("/omr/templates", response_model=list[OmrTemplateResponse])
async def get_omr_templates() -> list[OmrTemplateResponse]:
    return await list_omr_templates()

@router.get("/omr/templates/{template_id}", response_model=OmrTemplateResponse)
async def get_omr_template_endpoint(template_id: str) -> OmrTemplateResponse:
    return await get_omr_template(template_id)

@router.delete("/omr/templates/{template_id}")
async def delete_omr_template_endpoint(template_id: str) -> Response:
    await delete_omr_template(template_id)
    return Response(status_code=204)

@router.post("/omr/templates/{template_id}/detect", response_model=OmrDetectionResponse)
async def detect_omr_sheet_endpoint(template_id: str, sheet_id: str) -> OmrDetectionResponse:
    return await run_detection(template_id, sheet_id)

@router.get("/omr/detections/{sheet_id}", response_model=OmrDetectionResponse)
async def get_omr_detection_endpoint(sheet_id: str) -> OmrDetectionResponse:
    return await get_saved_detection(sheet_id)

@router.get("/omr/sheets/{sheet_id}/preview")
async def get_omr_sheet_preview_endpoint(sheet_id: str, template_id: str) -> Response:
    image_bytes = await render_preview_image(template_id, sheet_id)
...
async def override_omr_detection_endpoint(
    sheet_id: str, template_id: str, request: OmrDetectionOverrideRequest,
) -> OmrDetectionResponse:
    return await override_detection(sheet_id, template_id, request.sbd, request.made, request.answers)

@router.post("/omr/answer-keys", response_model=OmrAnswerKeyResponse)
async def create_omr_answer_key_endpoint(request: OmrAnswerKeyCreateRequest) -> OmrAnswerKeyResponse:
    return await create_answer_key(request)

@router.get("/omr/answer-keys", response_model=list[OmrAnswerKeyResponse])
async def get_omr_answer_keys_endpoint() -> list[OmrAnswerKeyResponse]:
    return await list_omr_answer_keys()

@router.get("/omr/answer-keys/{answer_key_id}", response_model=OmrAnswerKeyResponse)
async def get_omr_answer_key_endpoint(answer_key_id: str) -> OmrAnswerKeyResponse:
    return await get_omr_answer_key(answer_key_id)

@router.delete("/omr/answer-keys/{answer_key_id}")
async def delete_omr_answer_key_endpoint(answer_key_id: str) -> Response:
    await delete_omr_answer_key(answer_key_id)
    return Response(status_code=204)

@router.post("/omr/answer-keys/{answer_key_id}/grade-batch", response_model=OmrGradeBatchResponse)
async def grade_omr_batch_endpoint(answer_key_id: str, request: OmrGradeBatchRequest) -> OmrGradeBatchResponse:
    return await grade_sheets_batch(answer_key_id, request.sheet_ids)

@router.post("/omr/results", response_model=list[OmrGradedResultResponse])
async def save_omr_results_endpoint(request: OmrGradedResultSaveRequest) -> list[OmrGradedResultResponse]:
    return await save_graded_results(request)

@router.get("/omr/results", response_model=list[OmrGradedResultResponse])
async def get_omr_results_endpoint() -> list[OmrGradedResultResponse]:
    return await list_omr_graded_results()

@router.delete("/omr/results/{result_id}")
async def delete_omr_result_endpoint(result_id: str) -> Response:
    await delete_omr_graded_result(result_id)
    return Response(status_code=204)
